---
title: "El análisis de sentimientos"
author: "JMFR"
date: "11/5/2019"
...
Te voy a mostrar cómo hacer un análisis de sentimientos aplicado a textos literarios, aunque será muy básico. Todo el material que necesitas lo tienes en los subdirectorios datos –`episodios.rda` y `diccionarios.rda`– y codigo –`get_sentiments.R`–.
...
library(tidyverse)
library(tidytext)
load("datos/diccionarios.rda")
source("codigo/get_sentiments.R")
```
El siguiente paso es cargar los textos. Podrías hacerlo tomándolos de subdirectorio `datos/galdos` donde tienes una copia de las cuarentaiséis entregas que los constituyen, pero sería un poco enredado prepararlos todos y oscurecería las explicaciones. Por eso he preparado un fichero llamado `episodios.rda` que tiene las diez entregas que conforman la primera serie de los Episodios nacionales de Benito Pérez Galdós. Los tienes de una forma un tanto especial: convertidos en diez objetos –`arapiles`, `bailen`, `cadiz`, `carlos4`, `empecinado`, `gerona`, `marzo_mayo`, `napoleon`, `trafalgar`, y `zaragoza`– en los que cada capítulo es un elemento (`39`, `32`, `35`, `28`, `33`, `27`, `30`, `29`, `17` y `32` respectivamente). Lo único que tienes que hacer buscarlo en la pestaña `Files`, tiene que estar en el directorio `datos`. Una vez ahí haz clic en `episodios.rda`. Aparecerá en el borde superior de la ventana de `RStudio` un mensaje como el de la figura 11.2. Haz clic en `Yes`.

```{R 11-Sentimiento-02, echo = FALSE, out.width = '50%', fig.align='center', fig.cap = "Fig. 11.1. Aviso de carga de un fichero `RData`"}
knitr::include_graphics("imagenes/11-Sentimiento-02.png")
```

En seguida aparecerán en la ventana **Global Enviroment** diez vectores de caracteres llamados `arapiles`, `bailen`, `cadiz`, `carlos4`, `empecinado`, `gerona`, `marzo_mayo`, `napoleon`, `trafalgar`, y `zaragoza`. Puedes verlo con solo teclear en la consola
```{R eval=FALSE}
trafalgar
```

Deberá aparecer todo el texto; cada elemento será un capítulo, y dentro del elemento estará todo el texto del capítulo sin formato ni marcas de párrafo.

Ahora tienes que cargar dos diccionarios. Ahora haz lo mismo que antes con el fichero `diccionarios.rda`. Aparecerá la misma solicitud que antes. Pulsa `Yes`. Se cargará el fichero, solo que en esta ocasión no aparecerá en la subventana **Values**, como en el caso anterior, sino en la ventana **Data**. Ahí habrá dos tablas. Una llamada `sentimientos`, con 15452 observaciones y 4 variables y otra llamada `vacias_esp` con 765 observaciones.

Examina la tabla `sentimientos`. Escribe en la consola `sentimientos` y pulsa `intro`. Aparecerá el comienzo de una gran tabla, de 15452 líneas y cuatro columnas:
...
Se trata de la versión reducida del diccionario de sentimientos que hemos desarrollado para usarlo con el paquete `tidytext`. Tiene cuatro columnas: `palabra`, `sentimiento`, `lexicon` y `valor`. En realidad, hay tres diccionarios diferentes construidos de manera diferente y con diferente número de palabras[^1].

La columna `palabra` contiene las diferentes palabras de cada uno de los diccionarios (el nombre de cada uno de ellos está recogido en la columna lexicon y puede ser `nrc`, `afinn` y `bing`)1. La columna valor es siempre `NA` para `nrc` y `bing`, y un valor numérico entre `-5` y `+5` para `afinn`. En cambio, `afinn` tiene siempre un valor `NA` en la columna sentimiento, mientras que en `bing` puede ser `negativo` o `positivo` y en `nrc` uno de estos diez valores: `positivo`, `negativo`, `ira`, `miedo`, `tristeza`, `disgusto`, `asombro`, `confianza`, `alegría`, `promonición`. Algunas palabras pueden tener en `nrc` varios valores, pero siempre relacionados. Fíjate en el cuadro anterior, `abandonar` está marcado como `miedo`, `negativo` y `tristeza`.

La otra tabla, la llamada `vacias_es`, es, en realidad, la lista de palabras vacías que has utilizado en ocasiones anteriores. Nada nuevo.
...
Si no has jugueteado con lo que acabas de hacer, puedes proseguir, en caso contrario mi recomendación es que cierres `RStudio` e inicies una nueva sesión. A continuación carga las dos librerías necesarias:

```{R}
library(tidyverse)
```

La consola te habrá respondido con lo que aparece en las tres cajas anteriores. No hay ningún problema. `tidyverse`, como ya te he contado, es un conjunto de librerías, por lo que que te ahorra cargarlas individualmente y que te olvides de alguna en algún momento.
La tercera caja te informa de que hay algunos conflictos con algunos otros paquetes. Nada de lo que tengas que preocuparte. No te afectará.

```{R}
library(tidytext)
```

Ahora `episodios.rda` y `diccionarios.rda`, pero no como has hecho hasta ahora, sino con programación. Ambos están en el directorio `datos`:

```{R}
load("datos/episodios.rda")
load("datos/diccionarios.rda")
```

y por último el fichero `get_sentiments.R` que se encuentra en el directorio `codigo`:

```{R}
source("codigo/get_sentiments.R")
```
Este pequeño fichero de `R` contiene la función `get_sementiments` del paquete `tidytext` ligeramente modificada para que puentee los diccionarios que trae incluidos `tidytext` puesto que son inútiles para analizar un texto en español ya que se diseñó para el mundo de habla inglés. Ya tienes el entorno preparado. El aspecto de la ventana **Global Environment** debe ser el de la figura 11.3. Cuando quieras analizar textos en inglés no cargues los diccionarios españoles ni `get_sentiments.R`. Utiliza los que trae de serie la librería tidytext.

```{R 11-Sentimiento-03, echo = FALSE, out.width = '75%', fig.align='center', fig.cap = "La ventana **Global Enviroment** una vez cargados `episodios.rda`, `diccionarios.rda` y la función `get_sentiments()`"}
knitr::include_graphics("imagenes/11-Sentimiento-03.png")
```

Ahora tienes que preparar los textos para poderle seguir la pista a cada una de las palabras que lo constituyen. Ya los has cargado por medio del fichero `episodios.rda`, pero, como te he explicado en la sección anterior, son vectores de caracteres y esos no son los mejores objetos de `R` para usar el sistema `tidydata`, con lo que hay que convertirlos en una `tibble`.

Lo primero es crear un vector de caracteres con el título de cada entrega, llámalo, evidentemente, `titulo`. Créalo en el orden cronológico de publicación, así el análisis será mucho más interesante.
...
pero lo más seguro es que se salga por el lado derecho de la ventana del editor y no lo veas bien.

El siguiente paso es crear una lista, llamada `libros`, con todos los textos en el orden de publicación. Se crea con la función `list()` y cada uno de los elementos de la lista es cada uno de los vectores de caracteres que tienen el texto de cada entrega.
...
El siguiente paso es crear lo gran tabla, llamada `serie`, con todos los textos, con el título, el número de capítulo y cada una de las palabras convertidas en palabras-token, es decir, todas convertidas en letras minúsculas y sin ningún signo de puntuación. Como es una labor que se ha de llevar a cabo con un bucle `for`, lo primero es inicializar la `tibble` que guardará todo el material.
...
En la siguiente la función `mutate()` crea una nueva columna llamada `libro`, que contendrá el título de la entrega y lo extraerá del vector `titulos`.
...
Después la función `select()` se encarga de crear la tabla intermedia definitiva, la cual solo tendrá las tres columnas del paso anterior, pero reorganizadas: `libro`, que es el título, `capitulo`, que contiene el número de cada capítulo, y `palabra`, cada uno de las palabra-token que constituyen cada capítulo.
...
Ya tienes el material preparado. Una enorme tabla en la que están todas las palabras de las diez primeras novelas de los Episodios galdosianos. Es más, las sigues teniendo en el orden de lectura. Puedes comprobarlo con `serie$palabra[1:50]`. Si escribes esto en la consola y pulsas intro, se imprimirán en la pantalla las 50 primeras palabras.
...
Lo primero que vas a ver es cuántas palabras positivas y cuántas negativas hay en la toda esta primera serie de los Episodios nacionales. Vas a utilizar el diccionario `nrc` porque además de palabras positivas y negativas indicará las palabras que transmiten alguna de las ocho emociones básicas. El código es:
...
La línea de `filter()` lo que le dice que solo tome aquellas filas cuyo contenido en la columna `sentimiento` sea diferente de `NA` –`!is.na`–. La última línea, `count()`, lo que hace es contar cuántas veces aparece cada una de los sentimientos en la columna `sentimiento` y ordenarlas de mayor a menor de acuerdo con el número de ocurrencias –`sort = TRUE`–. El resultado con el diccionario `nrc` lo has visto un poco antes.

Si cambias el argumento de `get_sentiments()` de `nrc` a `bing`, el resultado será una tabla de 2 por 2 que te informará de cuántas palabras positivas y cuántas negativas hay en todo el corpus.
...
En varias ocasiones he mencionado que no se ha reorganizado el texto de las diez entregas porque puede ser interesante ver cómo evoluciona la valoración de sentimientos a lo largo de la trayectoria narrativa de cada una de ellas. La mejor manera de verlo es por medio de una gráfica que genera este fragmento de código (la explicación está un poco más abajo).
...
El resultado interno, no lo verás en la consola, es:
...
Las seis última líneas de la tabla enorme tabla serían
...
Apenas puedes apreciar los resultados, puesto que presenta, a causa de la frecuencia de aparición, las cuatro primeras palabras positivas y las seis negativas. Ya sabes cómo puedes ofrecerlo de manera gráfica con unas pocas líneas de código
...
```{R}
texto_entrada <- read_lines("datos/textos/cuatro_jinetes_apocalipsis.txt")
```
